...
class Themes
{
    ...
    public function __construct()
    {
        ...
        $this->themesPath = config('themes.themes_path', config('themes.view_path', null)) ?: config('view.paths')[0];
    ...
    public function set($themeName)
    {
        ...
        if (!$this->exists($themeName)) {
            ...
            if (!is_dir($themes_path . '/' . $themeName)) {
        ...
        Event::dispatch('zanysoft.laravel-theme.change', $theme);
        return $theme;
    }
    ...
    public function asset($path = null)
    {
        ...
        return asset($this->current()->url($path, false));
    }

    public function asset_path($path = null, $absolute = true)
    {
        $path = ltrim($path, '/');
        ...
        return $this->current()->path($path, $absolute);
    }

    // Return url of current theme
    public function url($filename, $absolute = true)
    {
        ...
        return $this->current()->url($filename, $absolute);
    }
